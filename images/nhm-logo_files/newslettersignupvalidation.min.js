...
$(document).ready(function(){$(".newslettersignup form").each(function(){var a=$(this);
$(this).validate({rules:{name:"required",email:{required:true,email:true},question:{maxlength:0}},showErrors:function(b,c){if(this.numberOfInvalids()>0){a.find(".errors").html("Please enter a valid email address and fill in the necessary fields.");
a.find(".errors").show();
a.find(".policy").hide()
}else{a.find(".errors").hide();
a.find(".errors").html("");
a.find(".policy").show()
}},submitHandler:function(b){$.ajax({crossDomain:true,async:false,type:$(b).attr("method"),url:$(b).attr("action"),data:$(b).serialize(),dataType:"json",success:function(c,d){if(c.success=="true"){window.location.href=c.thankyouURL
}else{a.find(".errors").html(c.errorMessage);
a.find(".errors").show();
a.find(".policy").hide()
}},error:function(d,e,c){a.find(".errors").html("There has been an error and we cannot subscribe you to the newsletter at this time.");
a.find(".errors").show();
a.find(".policy").hide()
}});
return false
